...
export class MinioHttpFlowRepository implements FluxRepository {
  private static COMPRESSED_FILE_EXTENSION = ".zip";
  ...
  public async recuperer(flow: FluxExtraction): Promise<string> {
    ...
    return this.flowClient.pull(this.configuration.ONISEP.FLUX_URL, logger);
  }
  ...
  public async enregistrer(
    ...
    this.loggerStrategy.get(flow.nom).info(`Starting to save extracted formations initiales from flow ${flow.nom}`);
    ...
    try {
      ...
      this.loggerStrategy.get(flow.nom).info(`End of saving extracted formations initiales from flow ${flow.nom}`);
    }
  }
